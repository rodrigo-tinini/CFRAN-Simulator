...
for i in range(number_of_nodes):
	if i == 0:
		dus_capacity.append(cloud_du_capacity)
	else:
		dus_capacity.append(fog_du_capacity)
#print("dus capacity", dus_capacity)
...
class Traffic_Generator(object):
	...
	def countIncBatchAverages(self):
		...
		if total_down_time:
			average_down_time.append(numpy.mean(total_down_time))
			total_down_time = []
		else:
	...
	def extSingleMigrations(self, ilp_module, copy_state):
		...
		for i in ilp_module.nodes:
			if copy_state[i] > ilp_module.rrhs_on_nodes[i] and copy_state[0] < ilp_module.rrhs_on_nodes[0]:
				...
				total_vm_migrations_time.append((diff*614.4)/migrating_vpons)
...
class Util(object):
	...
	def createRRHs(self, amount,env, service_time, cp, nodes_matrix):
		...
		for i in range(amount):
			...
			r = RRH(i, copy.copy(f_matrix), env, service_time, cp)
			rrhs.append(r)
		self.setMatrix(rrhs)
		return rrhs
